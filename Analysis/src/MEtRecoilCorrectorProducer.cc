...
void MEtRecoilCorrectorProducer::produce(edm::Event & iEvent, const edm::EventSetup & iSetup){
  ...
  iEvent.getByLabel(metTag_,metHandle);
  const reco::PFMETCollection* mets = metHandle.product();
  ...
  if(isMC_ && eventCounter_%10000==0){
    ...
    for(unsigned int k = 0; k < genParticles->size(); k ++){
      ...
      if( (gen.pdgId()== 35 or gen.pdgId()== 36 or gen.pdgId()== 25) && gen.status()==3 ) counterH++;
    }
  ...
  if(recoilCorr_!=0){
    ...
    */
    ...
    for(unsigned int k = 0; k < genParticles->size(); k ++){
      ...
      if( ! ((*genParticles)[k].pdgId() == 23 || abs((*genParticles)[k].pdgId()) == 24 ||(*genParticles)[k].pdgId() == 25 || (*genParticles)[k].pdgId() == 36 || (*genParticles)[k].pdgId() == 35) ||
